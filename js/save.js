...
function save() {//借鉴的
    addNotify("保存成功");
    localStorage.setItem("stocksH", formatsave.encode(player));
}
function load() {//+1
    addNotify("加载成功");
    let loadplayer = localStorage.getItem("stocksH");
    if (loadplayer != null) {
        let loadplayer = formatsave.decode(localStorage.getItem("stocksH"));
